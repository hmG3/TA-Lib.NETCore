...
public static partial class Functions<T> where T : IFloatingPointIeee754<T>
{
        ...
        out int outBegIdx, out int outNbElement, int optInTimePeriod = 14)
    {
        ...
        if (optInTimePeriod == 1)
        {
            ...
            while (today < endIdx)
            {
                ...
                if (diffP > T.Zero && diffP > diffM)
                {
                    ...
                    outReal[outIdx++] = !TA_IsZero(tempReal) ? diffP / tempReal : T.Zero;
                }
                else
                {
        ...
        outReal[0] = !TA_IsZero(prevTR) ? THundred * (prevPlusDM / prevTR) : T.Zero;
        outIdx = 1;
        ...
        while (today < endIdx)
        {
            ...
            prevTR = prevTR - prevTR / tOptInTimePeriod + tempReal;
            prevClose = inClose[today];
            outReal[outIdx++] = !TA_IsZero(prevTR) ? THundred * (prevPlusDM / prevTR) : T.Zero;
        }
    ...
    public static int PlusDILookback(int optInTimePeriod = 14) =>
        ...
        {
            < 1 or > 100000 => -1,
            _ => optInTimePeriod > 1 ? optInTimePeriod + Core.UnstablePeriodSettings.Get(Core.FuncUnstId.PlusDI) : 1
        };
}
