...
{
    public static Core.RetCode Ppo(T[] inReal, int startIdx, int endIdx, T[] outReal, out int outBegIdx, out int outNbElement,
    ...
    public static int PpoLookback(int optInFastPeriod = 12, int optInSlowPeriod = 26, Core.MAType optInMAType = Core.MAType.Sma) =>
        optInFastPeriod is < 2 or > 100000 || optInSlowPeriod is < 2 or > 100000
            ? -1
            : MaLookback(Math.Max(optInSlowPeriod, optInFastPeriod), optInMAType);
}
